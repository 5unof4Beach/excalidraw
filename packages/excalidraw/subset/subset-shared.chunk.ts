...
export const subsetToBinary = async (
  ...
  const snftSubset = subset(decompressedBinary as any, new Set(codePoints));
  const compressedBinary = compress(snftSubset.buffer as any);

  return compressedBinary.buffer as any;
};
